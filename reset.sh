#!/bin/bash
DBNAME=$(python -c 'from inyoka.conf import settings; print settings.DATABASE_NAME')
...
rm -rf $(python -c 'from inyoka.conf import settings; print "\"%s\"" % settings.XAPIAN_DATABASE')
python -c 'import xapian; from inyoka.conf import settings; xapian.WritableDatabase(settings.XAPIAN_DATABASE, xapian.DB_CREATE_OR_OPEN)' && echo "Created xapian database"
