...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_index('portal_user', ['username'])

        # Adding index on 'StaticFile', fields ['identifier']
        db.create_index('portal_staticfile', ['identifier'])

        # Adding index on 'SessionInfo', fields ['key']
        db.create_index('portal_sessioninfo', ['key'])

        # Adding index on 'SessionInfo', fields ['last_change']
        db.create_index('portal_sessioninfo', ['last_change'])

        # Adding index on 'Group', fields ['name']
        db.create_index('portal_group', ['name'])

        # Adding index on 'Event', fields ['date']
        db.create_index('portal_event', ['date'])
    ...
    def backwards(self, orm):
        ...
        db.delete_index('portal_group', ['name'])

        # Removing index on 'SessionInfo', fields ['last_change']
        db.delete_index('portal_sessioninfo', ['last_change'])

        # Removing index on 'SessionInfo', fields ['key']
        db.delete_index('portal_sessioninfo', ['key'])

        # Removing index on 'StaticFile', fields ['identifier']
        db.delete_index('portal_staticfile', ['identifier'])

        # Removing index on 'User', fields ['username']
        db.delete_index('portal_user', ['username'])

        # Removing index on 'User', fields ['email']
        db.delete_index('portal_user', ['email'])
