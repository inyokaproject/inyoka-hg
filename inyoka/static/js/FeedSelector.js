...
var FEED_COUNTS = [5, 10, 15, 20, 25, 50, 75, 100];
...
{
  $('#id_forum_forum').change(function () { document.getElementById('id_forum_component_forum').checked = true; });
  $('#forum').change(function () { make_forum_feed_url(); });
  $('#ikhaya').change(function () { make_ikhaya_feed_url(); });
  $('#planet').change(function () { make_planet_feed_url(); });
  $('.feed_count').after(' <span class="feed_count_adjust">');
  $('.feed_count_adjust')
    .append('<img class="feed_count_increase" src="' + $STATIC_URL + 'img/arrow-up.gif" />')
...
function make_forum_feed_url()
{
  var errors = {};
  var data = {};
  var form = document.getElementById('forum');
  
  if (form.component[0].checked)
    data.component = '*';
  else if (form.component[1].checked)
  {
    data.component = 'forum';
    if (form.forum.selectedIndex <= 0)
      errors.forum = 'Bitte ein Forum auswählen!';
    else
      data.forum = form.forum.options[form.forum.options.selectedIndex].value;
  }
  else
    errors.component = 'Ungültige Auswahl!';
  
  _get_count_mode(form, data, errors);  
  ...
  {
    if (data.component == '*')
      feed_url = FORUM_URL + 'feeds/' + data.mode + '/' + data.count + '/';
    else
      feed_url = FORUM_URL + 'feeds/' + data.component + '/' + data.forum + '/' + data.mode + '/' + data.count + '/';
    if (navigator.userAgent.substring(0,5).toLowerCase() != 'opera')
    {
      if ($('#forum_feed_url').length == 0)
      {
        $('#forum_submit_p input').addClass('hidden');
        $('#forum_submit_p').prepend('<strong>Adresse des Feeds:</strong> <a id="forum_feed_url" href="about:blank>-</a></span> ');
      }
...
function make_ikhaya_feed_url()
{
  ...
  if (OK)
  {
    ...
    if (navigator.userAgent.substring(0,5).toLowerCase() != 'opera')
    {
      if ($('#ikhaya_feed_url').length == 0)
      {
        $('#ikhaya_submit_p input').addClass('hidden');
        $('#ikhaya_submit_p').prepend('<strong>Adresse des Feeds:</strong> <a id="ikhaya_feed_url" href="about:blank>-</a></span> ');
      }
...
function make_planet_feed_url()
{
  var errors = {};
  var data = {};
  var form = document.getElementById('planet');
  ...
  if (OK)
  {
    ...
    if (navigator.userAgent.substring(0,5).toLowerCase() != 'opera')
    {
      if ($('#planet_feed_url').length == 0)
      {
        ...
        $('#planet_submit_p').prepend('<strong>Adresse des Feeds:</strong> <a id="planet_feed_url" href="about:blank>-</a></span> ');
      }
...
function _closest(n, l)
{
  ...
  {
    if (n < l[i])
      return (n - l[i-1] < l[i] - n && l[i-1] || l[i]);
  }
  return l[l.length-1];
}
...
function _get_count_mode(form, data, errors)
{
  ...
  if (isNaN(form.count.value) || form.count.value.length < 1)
  {
    ...
    form.count.value = data.count || '25';
  }
  else
    data.count = form.count.value = _closest(form.count.value, FEED_COUNTS);
}
