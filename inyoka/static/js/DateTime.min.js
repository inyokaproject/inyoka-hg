(function(){var l=["Januar","Februar","M\u00e4rz","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"],m=["S","M","D","M","D","F","S"];DateTimeField=Class.$extend({__init__:function(b,c,a){var e=this;this.input=$(b).click(function(){$("table.datetime").each(function(){e.container[0]!==this?$(this).hide():e.show()});return false});this.auto_show=c||false;this.only_date=a||false;this.readDateTime();this.container=$('<table class="datetime"></table>').click(function(){return false});
...
$(this).next().focus()}).attr("title","Klicke hier, um schnell einen anderen Monat auszuw\u00e4hlen")))),f=$("<tbody></tbody>").appendTo(e),d=$("<tr></tr>").appendTo(f);$.each(m,function(k,n){d.append($('<th class="weekday"></th>').text(n))});var g=(new Date(a,c-1,1)).getDay(),j=$.inArray(c,[1,3,5,7,8,10,12])!=-1?31:$.inArray(c,[4,6,9,11])!=-1?30:c==2&&(a%4===0&&a%100!==0||a%400===0)?29:28;d=$("<tr></tr>").appendTo(f);for(var h=0;h<g;h++)$('<td style="background-color: #f3f3f3;"></td>').appendTo(d);
var i=1,o=function(){$(".selected",$(this).parent().parent().parent()).removeClass("selected");$(this).parent().addClass("selected");b.currentDay=$(this).text();b.currentMonth=c;b.currentYear=a;b.writeDateTime();return false};for(h=g;i<=j;h++){if(h%7===0&&i!=1)d=$("<tr></tr>").appendTo(f);g=$("<td></td>").append($("<a></a>").text(i).click(o)).appendTo(d);a==this.currentYear&&c==this.currentMonth&&i==this.currentDay&&g.addClass("selected");i++}for(;d.children().length<7;)d.append($('<td class="nonday"></td>'));
this.calendar.append(e)},drawDate:function(b,c,a){a=parseInt(a,10);c=parseInt(c,10);b=parseInt(b,10);if(c>0&&c<13){this.calendarMonth=c;this.calendarYear=b;if(a){this.currentDay=a;this.currentMonth=c;this.currentYear=b}this.drawCalendar()}},drawPreviousMonth:function(){if(this.calendarMonth==1){this.calendarMonth=12;this.calendarYear--}else this.calendarMonth--;this.drawCalendar()},drawNextMonth:function(){if(this.calendarMonth==12){this.calendarMonth=1;this.calendarYear++}else this.calendarMonth++;
this.drawCalendar()},drawPreviousYear:function(){this.calendarYear--;this.drawCalendar()},drawNextYear:function(){this.calendarYear++;this.drawCalendar()},destroy:function(){this.writeDateTime();this.container.remove();this.input.show()}});$("input").each(function(){var b=this.getAttribute("valuetype");if(b=="datetime")DateTimeField(this,true);else b=="date"&&DateTimeField(this,true,true)});$(document).click(function(){$("table.datetime.auto_show").hide()})})();
