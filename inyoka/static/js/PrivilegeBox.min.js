...
$.each(b,function(c,n){$("#forum_"+n).addClass("active")});if(b.length==1){var f=b[0];r.text($("#forum_"+f).text());$.each(p,function(c){c=parseInt(c);var n;n=$.inArray(c,g.mapping[f])>-1?"1":$.inArray(c*-1,g.mapping[f])>-1?"-1":"0";$("#priv_"+c+"_"+n).attr("checked","checked")})}else{r.text(b.length+" Foren");$("input[type=radio]",g.container).attr("checked","")}}))});this.container.html("");this.container.append(o);j=$('<div class="privileges"></div>').appendTo(this.container);var r=$("<h5 />").appendTo(j),
t=$("<dl />").appendTo(j);$.each(p,function(l,i){var d=function(k,m){return $('<input type="radio" />').attr({name:"priv_"+l,value:k,id:"priv_"+l+"_"+k}).change(function(){var h=[];g.container.find("input").each(function(e,a){if(a.name.substring(0,5)=="priv_"&&a.checked){var f=parseInt(a.name.substring(5));if(q=parseInt(a.value)*f)h[h.length]=q}});$.each(b,function(e,a){var f="forum_privileges_"+a,c=$("input[name="+f+"]");if(c.length===0)c=$('<input type="hidden" />').attr("name",f).appendTo(g.container);
