...
Class.$extend({__init__:function(a,c){var b=this,d;this.profile=c||"small";this.username=$CURRENT_USER||"Anonymous";this.smilies=null;this.textarea=$(a);this.textarea[0].inyokaWikiEditor=this;$(this.textarea).keydown(function(l){b.keycodes(b,l)});d=$('<ul class="toolbar" />').prependTo(this.textarea.parent());for(var e=q(),f,h=0,m=e.length;h!=m;++h)f=e[1](b);f&&f.appendTo($("<li />").appendTo(d));$('<span class="syntax_help note"><a href="'+(c=="wiki"?"http://wiki.ubuntuusers.de/Wiki/Syntax":"http://wiki.ubuntuusers.de/Forum/Syntax")+
'">Hilfe zur Syntax</a></span>').appendTo($("<li />").appendTo(d))},keycodes:function(a,c){if(c.keyCode==13){var b=a.getCurrentLine().match(/^(\s*(?:\*|- |[01aAiI]\.)?\s*)(.*?)$/);if(b[1].length){c.preventDefault();b[2].length?this.insertText("\n"+b[1]):this.setCurrentLine("\n")}}else if(c.keyCode==9){c.preventDefault();b=a.getCurrentLine().length;for(var d=(Math.floor(b/2)+1)*2,e="";b<d&&(e+=" ");++b)this.insertText(e)}},insertTag:function(a,c){var b=this.textarea[0],d=a instanceof Array?a:a.split("%s",
2),e=d[0]||"";d=d[1]||"";scroll=b.scrollTop;if(typeof b.selectionStart!="undefined"){var f=b.selectionStart,h=b.selectionEnd,m=b.value.substring(0,f),l=b.value.substring(f,h),r=b.value.substring(h);l=h!=f?e+l+d:e+c+d;b.value=m+l+r;b.focus();b.selectionStart=f+e.length;b.selectionEnd=f+(l.length-d.length)}else if(typeof document.selection!="undefined"){b.focus();f=document.selection.createRange();h=f.text;f.text=e+(h.length>0?h:c)+d}b.scrollTop=scroll},getSelection:function(){var a=this.textarea[0];
